/**
 * Booking Module
 * Handles car rentals with two booking modes: with-driver and minicab ride-hailing
...
(function initBookingModule() {
  ...
  document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (BOOKING_MODE === 'minicab') {
      ...
      const loadingEl = document.getElementById('bookingLoading');
      if (loadingEl) loadingEl.style.display = 'none';
      const noCarEl = document.getElementById('bookingNoCar');
      if (noCarEl) noCarEl.style.display = 'block';
      return;
    } else {
      try {
  ...
  function selectBookingType(type) {
    ...
    if (isMinicab) {
      selectRideTiming(selectedRideTiming);
    }
  ...
  function selectRideTiming(timing) {
    ...
    if (timing === 'now') {
      ...
      if (scheduledDateTimeGroup) scheduledDateTimeGroup.style.display = 'none';
      
      const now = new Date();
      const pickupDate = document.getElementById('pickupDate');
      if (pickupDate) pickupDate.value = now.toISOString().split('T')[0];
    } else {
      if (pickupDateGroup) pickupDateGroup.style.display = 'none';
      if (returnDateGroup) returnDateGroup.style.display = 'none';
  ...
  async function goToStep2() {
    ...
    if (selectedBookingType === 'with-driver') {
      ...
      if (selectedRideTiming === 'schedule') {
        ...
        const pickupDate = document.getElementById('pickupDate');
        if (pickupDate) pickupDate.value = new Date().toISOString().split('T')[0];
      }
    ...
    if (selectedBookingType === 'minicab') {
      ...
      if (paymentPickupDate) {
        if (selectedRideTiming === 'now') {
          paymentPickupDate.textContent = '⚡ Ride Now — ' + new Date().toLocaleString('en-US', { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        } else {
          const scheduled = document.getElementById('scheduledDateTime');
          const scheduledVal = scheduled ? scheduled.value : '';
          paymentPickupDate.textContent = scheduledVal ? new Date(scheduledVal).toLocaleString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : formatDate(pickupVal);
        }
      }
