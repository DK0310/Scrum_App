...
async function loadProfile() {
    try {
        ...
        // Role badge - support 5 roles (user/driver/staff/admin/owner)
        let roleBadge;
        switch(u.role) {
            case 'admin': roleBadge = '⚙️ Administrator'; break;
            case 'staff': roleBadge = '👨 Staff Manager'; break;
            case 'driver': roleBadge = '🚗 Driver'; break;
            case 'user': roleBadge = '👤 Customer'; break;
            case 'owner': roleBadge = '🏢 Vehicle Owner'; break;
...
async function doSaveProfile() {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (['owner', 'staff', 'admin'].includes(newRole)) {
                // Show My Vehicles link - staff/admin/owner can manage vehicles
                if (!myVehiclesLink) {
            ...
            } else if (['user', 'driver', 'renter'].includes(newRole)) {
                // Remove My Vehicles link for customer/driver roles
                if (myVehiclesLink) myVehiclesLink.parentElement.remove();
            }

            const roleBadge = newRole === 'admin' ? '⚙️ Administrator' 
                : newRole === 'staff' ? '👨 Staff Manager'
                : newRole === 'driver' ? '🚗 Driver'
                : newRole === 'user' ? '👤 Customer'
                : newRole === 'owner' ? '🏢 Vehicle Owner' : '👤 Customer';
            document.getElementById('profileRole').textContent = roleBadge;
