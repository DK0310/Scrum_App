...
function renderAdminUsers(users) {
    ...
    const roleColors = { 'admin': '#ef4444', 'user': '#3b82f6', 'driver': '#f59e0b', 'staff': '#8b5cf6' };
...
function adminChangeRole(id, currentRole, name) {
    const newRole = prompt(`Change role for "${name}":\nCurrent: ${currentRole}\n\nEnter new role (user, driver, staff, admin):`, currentRole);
    if (!newRole || newRole === currentRole) return;
    if (!['user', 'driver', 'staff', 'admin'].includes(newRole)) {
        adminNotify('Invalid role. Use: user, driver, staff, or admin.', 'error');
        return;
    }
